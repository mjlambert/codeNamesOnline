...
// assign words a type (blue|red|none|kill),
...
function buildGrid (words, callback) {
	...
	var types = [
		{ name : 'blue team', count : 8 },
		{ name : 'red team',  count : 8 },
		{ name : 'no team',   count : 8 },
		{ name : 'kill word', count : 1 }
	];
	...
	for (var x = 0; x < 5; x++) {
		for (var y = 0; y < 5; y++) {
			var typeIndex = Math.floor(Math.random() * (types.length - 1));
			grid[x][y] = {
				word   : words.pop(),
