...
module.exports.CreateNewGame = function (callback) {
	...
	wordGridGenerator.generateWordGrid(function (error, wordGrid) {
		if (error) {
			...
			currentGames.push(new Game(gameCode, wordGrid));
			return callback(null, gameCode);
		}
	});
};

module.exports.AddSocket = function (gameCode, socket) {
	var game = GetGame(gameCode);
	if (game) {
		game.setSocket(socket);
	}
	else {
		throw new Error('Cannot add socket. Game \'' + gameCode + '\' does not exist');
	}
};
