...
module.exports.CreateNewGame = function (socketId, callback) {
	...
	wordGridGenerator.generateWordGrid(function (error, wordGrid) {
		if (error) {
			...
			currentGames.push(new Game(gameCode, wordGrid.grid, wordGrid.startTeam, socketId));
...
module.exports.SelectWord = function (gameCode, word) {
	var game = GetGame(gameCode);
	if (game) {
		game.selectWord(word);
	}
	else {
		throw new Error('Cannot select word. Game \'' + gameCode + '\' does not exist');
	}
};

module.exports.NextTurn = function (gameCode) {
	var game = GetGame(gameCode);
	if (game) {
		game.nextTurn();
	}
	else {
		throw new Error('Cannot go to next turn. Game \'' + gameCode + '\' does not exist');
	}
};
...
module.exports.ResetGame = function (gameCode, callback) {
	var game = GetGame(gameCode);
	if (!game) {
		throw new Error('Cannot reset game. Game \'' + gameCode + '\' does not exist');
	}
	wordGridGenerator.generateWordGrid(function (error, wordGrid) {
		if (error) {
			return callback(error);
			...
			game.resetGame(wordGrid.grid, wordGrid.startTeam);
			return callback();
		}
	});
};
